...
func main() {
	...
	m, err := mecab.New("-d /usr/share/mecab/dic/mecab-ipadic-neologd")
	if err != nil {
		log.Fatal(err)
	}
	defer m.Destroy()
	...
	for pi, p := range precures {
		for si, s := range p.Subtitles {
			...
			var keitaiso []string
			for node := tg.ParseToNode(lt); node.Next() == nil; {
				keitaiso = append(keitaiso, node.Surface())
			}
			precures[pi].Subtitles[si].Keitaiso = keitaiso
		}
	}
